...
final class SendWhatsAppInteractiveProductMessageTest extends TestCase
{
    public function testApiCallExpectsSuccess(): void
    {
        // arrange
        $resource = $this->getResource();
        $mockedResponse = $this->loadJsonDataFixture('Endpoints/WhatsApp/send_whatsapp_interactive_product_message_success.json');
        ...
        $response = $this
            ->getInfobipClient()
            ->whatsApp()
            ->sendWhatsAppInteractiveProductMessage($resource);
    ...
    public function testApiCallExpectsBadRequestException(): void
    {
        // arrange
        $resource = $this->getResource();
        $mockedResponse = $this->loadJsonDataFixture('Endpoints/WhatsApp/send_whatsapp_interactive_product_message_bad_request.json');
    ...
    private function getResource(): WhatsAppInteractiveProductMessageResource
    {
        return new WhatsAppInteractiveProductMessageResource(
            'from',
            'to',
            new InteractiveProductContent(
                new InteractiveProductBody('text'),
                new InteractiveProductAction('catalogId', 'productRetailerId')
            )
        );
    }
}
