...
final class SendWhatsAppInteractiveListMessageTest extends TestCase
{
    public function testApiCallExpectsSuccess(): void
    {
        // arrange
        $resource = $this->getResource();
        $mockedResponse = $this->loadJsonDataFixture('Endpoints/WhatsApp/send_whatsapp_interactive_list_message_success.json');
        ...
        $response = $this
            ->getInfobipClient()
            ->whatsApp()
            ->sendWhatsAppInteractiveListMessage($resource);
    ...
    public function testApiCallExpectsBadRequestException(): void
    {
        // arrange
        $resource = $this->getResource();
        $mockedResponse = $this->loadJsonDataFixture('Endpoints/WhatsApp/send_whatsapp_interactive_list_message_bad_request.json');
    ...
    private function getResource(): WhatsAppInteractiveListMessageResource
    {
        return new WhatsAppInteractiveListMessageResource(
            'from',
            'to',
            new InteractiveListContent(
                new InteractiveListBody('text'),
                new InteractiveListAction('title')
            )
        );
    }
}
