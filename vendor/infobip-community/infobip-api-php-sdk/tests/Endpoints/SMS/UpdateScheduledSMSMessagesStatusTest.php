...
final class UpdateScheduledSMSMessagesStatusTest extends TestCase
{
    public function testApiCallExpectsSuccess(): void
    {
        // arrange
        $resource = $this->getResource();
        $mockedResponse = $this->loadJsonDataFixture('Endpoints/SMS/update_scheduled_sms_messages_status_success.json');
        ...
        $response = $this
            ->getInfobipClient()
            ->SMS()
            ->updateScheduledSMSMessagesStatus($resource);
    ...
    private function getResource(): UpdateScheduledSMSMessagesStatusResource
    {
        return new UpdateScheduledSMSMessagesStatusResource(
            'bulkId',
            new StatusType(StatusType::PAUSED)
        );
    }
}
