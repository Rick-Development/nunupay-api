...
class ActivityByUser
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if (Auth::guard('superadmin')->check()) {
            $expiresAt = Carbon::now()->addMinutes(2); // keep online for 1 min
            Cache::put('admin-is-online-' . Auth::guard('superadmin')->user()->id, true, $expiresAt);
            Admin::where('id', Auth::guard('superadmin')->user()->id)->update(['last_seen' => (new \DateTime())->format("Y-m-d H:i:s")]);
        }
        return $next($request);
    }
}
